    ...
    if df.empty:
        ...
        if not col_cliente or not col_produto:
            ...
            if not servicos:
                ...
                servico_sel = st.selectbox("Selecione o serviço", servicos)

                df_pdf = df[
                    df[col_produto].astype(str).str.strip().str.lower() == servico_sel.strip().lower()
                ].copy()

                if df_pdf.empty:
                    st.warning("Nenhum registro encontrado para esse serviço.")
                else:
                    ...
                    st.metric("Total do serviço selecionado", formatar_valor_brl(total_pdf))

                    pdf_buffer = criar_pdf_servico(df_pdf[["cliente", "servico", "valor_num"]], servico_sel)

                    nome_arquivo = f"relacao_{servico_sel.lower().replace(' ', '_')}.pdf"
                    st.download_button(
                        label="📥 Baixar PDF",
                        data=pdf_buffer,
