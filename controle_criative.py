...
# ===== CONFIGURAÇÕES =====
CAMINHO_JSON =  r"C:\Users\denil\Desktop\projetos\controle-442619-1a5ef1da59f3.json"
...
def conectar_google():
    ...
    creds = Credentials.from_service_account_file(CAMINHO_JSON, scopes=scopes)
    return creds
...
def carregar_dados():
    try:
        ...
        if "valor" in df.columns:
            df["valor_num"] = df["valor"].apply(converter_valor_para_float)
        else:
            df["valor_num"] = 0.0
...
def salvar_lancamento(data, revenda, cliente, valor, forma_pagamento, produto, situacao, texto_completo):
    try:
        ...
        link_doc = criar_documento_google_docs(titulo_doc, texto_completo or "")
...
if menu == "📋 Dashboard":
    ...
    if df.empty:
        ...
        with tab3:
            for _, row in df.iterrows():
                ...
                with st.expander(f"📝 ID {row['id']} - {row['cliente']} - {data_formatada}"):
    ...
    if df.empty:
        ...
        if tipo_busca == "ID":
            ...
            if busca:
                df_filtrado = df[df["cliente"].astype(str).str.contains(busca, case=False, na=False)]
            else:
                df_filtrado = df
            ...
            if busca:
                df_filtrado = df[df["revenda"].astype(str).str.contains(busca, case=False, na=False)]
            else:
                df_filtrado = df
        ...
        if df_filtrado.empty:
            ...
            with st.form("edit_form"):
                ...
                with col1:
                    ...
                    nova_revenda = st.text_input("Revenda", value=lanc["revenda"])
                    novo_cliente = st.text_input("Cliente", value=lanc["cliente"])
                ...
                with col2:
                    ...
                    forma_atual = lanc["forma de pagamento"]
