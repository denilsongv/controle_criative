...
if menu == "📋 Dashboard":
    ...
    with st.form("novo_form"):
        ...
        with col2:
            ...
            situacao = st.selectbox("Situação", ["entregue", "pendente", "cancelado", "em andamento"])
    ...
    if df.empty:
        ...
        if df_filtrado.empty:
            ...
            with st.expander("Visualizar dados atuais", expanded=False):
                ...
                st.text_area("Prévia do texto", texto_curto[:500] + ("..." if len(texto_curto) > 500 else ""), height=150, disabled=True)
            ...
            with st.form("edit_form"):
                ...
                with col2:
                    nova_forma = st.selectbox("Forma de pagamento", 
                        ["Dinheiro", "cartão de credito", "pix", "aguardando pagamento"],
                        index=["Dinheiro", "cartão de credito", "pix", "aguardando pagamento"].index(lanc['forma de pagamento']))
                    opcoes_produto = ["disa e-mail", "espera e-mail", "espera modulo", "disa chip", 
                                      "regravação espera", "natal", "itcp", "disa e-mail espera em chip", 
                                      "disa e espera por e-mail", "visita técnica", "Outro"]
                ...
                novo_texto = st.text_area("Texto completo (deixe em branco para manter o atual)", 
                                          value=texto_atual, height=200,
