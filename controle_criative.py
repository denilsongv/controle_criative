...
def criar_documento_google_docs(titulo, texto):
    """Cria documento no Google Docs. O texto não pode ser vazio."""
...
def salvar_lancamento(data, revenda, cliente, valor, forma_pagamento, produto, situacao, texto_completo):
    ...
    if not texto_completo or not texto_completo.strip():
        st.error("O texto do documento é obrigatório. Preencha o campo 'Texto completo'.")
        return None
    
    sheet = conectar_planilha()
    df = carregar_dados()
    
    # Calcular novo ID baseado no maior ID existente
...
if menu == "📋 Dashboard":
    ...
    with st.form("novo_form"):
        ...
        with col2:
            ...
            forma_pagamento = st.selectbox("Forma de pagamento", opcoes_pagamento, index=3)  # "Transferência pix"
            opcoes_produto = [
