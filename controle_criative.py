...
NOME_ABA = "criative"
ID_PASTA_NO_DRIVE = "1IdIqXjIc4oWd6Ohpg4Oef3lIPGPjUMvJ"  # Pasta compartilhada com a service account
...
def criar_documento_google_docs(titulo, texto):
    ...
    try:
        ...
        # Cria o documento dentro da pasta compartilhada
        file_metadata = {
            'name': titulo,
            'mimeType': 'application/vnd.google-apps.document',
            'parents': [ID_PASTA_NO_DRIVE]
        }
...
def atualizar_lancamento(id_lancamento, data, revenda, cliente, valor, forma_pagamento, produto, situacao, novo_texto):
    ...
    if novo_texto and novo_texto.strip():
        ...
        if not novo_link:
            st.error("Erro ao criar novo documento. Lançamento não atualizado.")
            return False
    else:
